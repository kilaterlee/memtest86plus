...
Change directory into the `build/ia32` directoryand type `make`. The result
...
Change directory into the `build/x64` directory and type `make`. The result
...
packages `grub-pc-bin`, `grub-efi-ia32-bin` and `grub-efi-amd64-bin`). You
may need to adjust some path and file names in the Makefile to match the
naming on your system.
